...
class RotatryPositionalEmbedding(nn.Module):
    ...
    def forward(self, x, pos_ids):
        # x shape: (batch, seq_len, d_k) or (batch, num_heads, seq_len, d_k)
        # pos_ids shape: (batch, seq_len)

        cos = self.cos_vals[pos_ids]  # (batch, seq_len, d_k/2)
        sin = self.sin_vals[pos_ids]  # (batch, seq_len, d_k/2)

        # Handle cases where x has 3 or 4 dimensions
        if x.dim() == 4:  # (batch, num_heads, seq_len, d_k)
